...
### `/audit-project` follow-through — all deferred items addressed

Went back through every remaining medium/high finding and the three largest deferred items (batched recalculation, first test suite, MapView hook refactor + keyboard accessibility) instead of stopping at critical/high. Four more commits: "Fix remaining medium/high findings," "Add first real test suite," "Refactor MapView into hooks; add keyboard-accessible segment list." Full detail in each commit message; `ROADMAP.md`'s "`/audit-project` findings — resolved" section has the summary and the handful of items still deliberately deferred (RouteSuggestion's keyboard-inaccessible endpoint picking, routing-graph bbox restriction, FK deletion strategy, DB/Redis-backed integration tests).

One real bug caught mid-fix, worth recording: the audit's suggestion to drop `segments` from `MapView`'s click-listener dependency array (to reduce unnecessary re-attachment) would have silently broken click-to-select on first load — the per-layer listeners only attach successfully if the MapLibre layers already exist, but `SegmentLayer` creates those layers asynchronously from that same `segments` state, so the dependency was load-bearing, not incidental. Fixed properly instead by switching to a plain map-level handler that queries whichever layers currently exist at click time (`map.queryRenderedFeatures`), which needed no dependency on `segments` at all. A reminder that audit findings describe symptoms accurately more often than they prescribe correct fixes — verify the "obvious" fix against the actual ordering/lifecycle constraints before applying it.
